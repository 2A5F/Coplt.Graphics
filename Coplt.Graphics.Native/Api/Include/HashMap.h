...
namespace Coplt
{
    ...
    template <class Key, Hash<Key> Hash , Eq<Key> Eq>
    class HashMap<Key, void, Hash, Eq> final
    {
        using HashHelpers = _HashMap_internal::HashHelpers;
        using Slot = _HashMap_internal::Slot;
    ...
    private:
        ...
        std::span<Key> Keys() const
        {
            return std::span(m_p_keys, m_cap);
        }
    ...
    public:
        ...
        ~HashMap()
        {
            ...
            if (m_p_keys != nullptr)
            {
                if constexpr (!std::is_trivially_destructible<Key>())
                {
                    auto iter = Iterator();
                    while (auto key = iter.Next())
                    {
                        key->~Key();
                    }
                }
                mi_free_aligned(m_p_keys, alignof(Key));
            }
        }
        ...
        HashMap(HashMap&& other) noexcept
            : m_p_buckets(std::exchange(other.m_p_buckets, nullptr)),
              m_p_slots(std::exchange(other.m_p_slots, nullptr)),
              m_p_keys(std::exchange(other.m_p_keys, nullptr)),
              m_fast_mod_multiplier(std::exchange(other.m_fast_mod_multiplier, 0)),
              m_cap(std::exchange(other.m_cap, 0)),
              m_count(std::exchange(other.m_count, 0)),
              m_free_list(std::exchange(other.m_free_list, 0))
    ...
    private:
        i32 Initialize(const i32 capacity)
        {
            ...
            m_p_buckets = static_cast<i32*>(mi_zalloc_aligned(size * sizeof(i32), alignof(i32)));
            m_p_slots = static_cast<Slot*>(mi_zalloc_aligned(size * sizeof(Slot), alignof(Slot)));
            m_p_keys = static_cast<Key*>(mi_malloc_aligned(size * sizeof(Key), alignof(Key)));
        ...
        void Resize(i32 new_size)
        {
            ...
            m_p_buckets = static_cast<i32*>(mi_zalloc_aligned(new_size * sizeof(i32), alignof(i32)));
            m_p_slots = static_cast<Slot*>(mi_rezalloc_aligned(m_p_slots, new_size * sizeof(Slot), alignof(Slot)));
            m_p_keys = static_cast<Key*>(
                mi_realloc_aligned(m_p_keys, new_size * sizeof(Key), alignof(Key))
            );
            m_fast_mod_multiplier = HashHelpers::GetFastModMultiplier(static_cast<u32>(new_size));
    ...
    public:
        // 返回是否成功添加，false 表示已经存在
        template <Fn<const Key&> QueryKey, Fn<void, UP<Key>> CreateKey>
        bool Add(QueryKey query_key, CreateKey create_key)
        {
            if (m_p_buckets == nullptr) Initialize(0);
            ...
            {
                auto& slot = m_p_slots[i];
                auto& slot_key = m_p_keys[i];
                if (slot.hash_code() == hash_code && m_eq(slot_key, key))
                {
                    return false;
                }
        ...
        bool Contains(const QKey& query_key, QHash hasher, QEq eq) const
        {
            ...
            do
            {
                ...
                slot = std::addressof(m_p_slots[i]);
                slot_key = std::addressof(m_p_keys[i]);
                if (slot->hash_code() == hash_code && eq(*slot_key, query_key))
                {
                    return true;
                }
        ...
        bool Contains(const Key& key) const
        {
            return Contains(key, m_hash, m_eq);
        }

        // 返回是否删除，如果删除并且 out_key 不为空将写入 out_key
        template <class QKey, Coplt::Hash<QKey> QHash = std::hash<QKey>, Coplt::Eq<Key, QKey> QEq = std::equal_to<Key>>
        bool Remove(const QKey& query_key, QHash hasher, QEq eq, Key* out_key)
        {
            if (m_p_buckets == nullptr) return false;
            u32 collision_count = 0;
            auto hash_code = hasher(query_key);
        ...
        // 返回是否删除，如果删除并且 out_key 不为空将写入 out_key
        bool Remove(const Key& key, Key* out_key)
        {
            return Remove(key, m_hash, m_eq, out_key);
        }
        ...
        public:
            ...
            Key* Next()
            {
                while (static_cast<u32>(m_index) < static_cast<u32>(m_self->m_count))
                {
                    auto i = m_index++;
                    auto& slot = m_self->m_p_slots[i];
                    auto& slot_key = m_self->m_p_keys[i];

                    if (slot.next >= -1)
                    {
                        return std::addressof(slot_key);
                    }
                }
        ...
        public:
            ...
            Key& operator*() const
            {
                return m_self->m_p_keys[m_index];
            }
